<template>
  <div class="px-4">
    <h1 class="text-4xl font-medium my-4">Elements</h1>
    <div class="h-0.5 mb-8 bg-gray-200" />
    <div
      class="flex items-center gap-4 px-4 py-3 hover:shadow-xl hover:cursor-pointer hover:bg-white rounded-md transition-all"
    >
      <span
        class="bg-green-200 flex items-center justify-center p-3 rounded-md text-2xl font-semibold text-green-600"
        >Aa</span
      >
      <div>
        <p class="text-lg font-bold">Text Input</p>
        <span class="text-sm">Single text input</span>
      </div>
    </div>
  </div>
</template>

<script setup lang="ts"></script>

<style scoped></style>
